...
const NotificationItem = ({
  ...
  >
    <div className="flex justify-between items-start">
      ...
      <span className={`text-xs opacity-80 ${textColor}`}>{notification.timestamp.toLocaleDateString()}</span>
    </div>
...
export const NotificationPopover = ({
  ...
  return (
    <div className={`relative ${textColor}`}>
      <button onClick={toggleOpen} className={cn("relative w-10 h-10 rounded-xl flex items-center justify-center", buttonClassName)}>
        <Bell size={16} />
        {unreadCount > 0 && (
          ...
          >
            <div className={`p-4 border-b ${headerBorderColor} flex justify-between items-center`}>
              ...
              <button
                ...
                className={`text-xs px-3 py-1.5 rounded-md bg-transparent ${hoverBgColor} hover:text-white transition-colors`}
              >
                Mark all as read
              </button>
            </div>
